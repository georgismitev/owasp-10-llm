...
- **Hard-label OR (measured here).** Max recall, zero tuning. But it also **unions the false positives** — whenever *either* model over-blocks a benign prompt, so does the ensemble. We never saw that cost: the trial has 1 benign control and both passed it. So the OR *looks* free and isn't proven to be.
